...
class ROS2CameraNode(Node):
    def __init__(self, node_name: str, topic: str, target_size: tuple = None):
        ...
        msg_type = CompressedImage if self.is_compressed else Image
        
        self.subscription = self.create_subscription(
            msg_type,
            topic,
            self.image_callback,
            qos_profile_sensor_data  # Use sensor data QoS (Best Effort) instead of Reliable to match publisher
        )
        logger.info(f"ROS2 Camera Node '{node_name}' subscribed to '{topic}' (Resize: {target_size})")

    def image_callback(self, msg: Any):
        ...
        if self.target_size is not None:
            try:
                ...
                frame = frame[:h, :w].copy()
        else:
            frame = frame.copy()
            
        # Convert BGR encoding to RGB since LeRobot expects RGB
        if msg_encoding == 'bgr8':
            # Use ascontiguousarray to prevent negative stride / FFMPEG encoder crashes
...
class ROS2Camera(Camera):
    ...
    def async_read(self, timeout_ms: float = 200) -> np.ndarray:
        ...
        # Wait for a new frame. Do not clear the event BEFORE waiting,
        # otherwise in a multi-camera setup, sequential reads will cause
        # "wooden barrel effect" (waiting for 1 frame per camera, dropping FPS).
        ...
        # Clear the event AFTER grabbing the frame, just like OpenCVCamera.
