...
class Tac3DTactileSensor(TactileSensor):
    ...
    def disconnect(self) -> None:
        ...
        if self._sensor:
            try:
                # The PyTac3D.Sensor class doesn't have a close() method,
                # but its internal UDP_Manager does.
                if hasattr(self._sensor, '_UDP'):
                    self._sensor._UDP.close()
                elif hasattr(self._sensor, 'close'):
                    self._sensor.close()
            except Exception as e:
    ...
    def _on_frame_received(self, frame: dict, callback_param: Any) -> None:
        ...
        try:
            ...
            # **CRITICAL FIX**: Do not perform periodic GC here!
            # The original implementation might have had garbage collection causing hitches.
            # Numpy arrays manage their own memory well in this tight loop.
