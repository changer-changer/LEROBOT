...
class PointCloud2TactileSensor(TactileSensor):
    ...
    def disconnect(self) -> None:
        ...
        # Note: This implementation doesn't use an explicit executor but rclpy.spin_once
        # The node should be destroyed after we stop calling spin
        node_to_destroy = self._node
        self._node = None  # This will cause the spin thread loop to exit

        if node_to_destroy:
            try:
                node_to_destroy.destroy_node()
            except Exception as e:
