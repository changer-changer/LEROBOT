...
class Tron2Robot(Robot):
    ...
    def connect(self):
        ...
        for name, cam in self.cameras.items():
            print(f"Starting ROS2 Camera: {name} ({cam.config.topic})...")
            cam.connect()
            print(f"ROS2 Camera {name} is READY.")
    ...
    def get_observation(self) -> dict:
        ...
        for name, cam in self.cameras.items():
            try:
                # Standard wait-and-read
                obs[name] = cam.async_read(timeout_ms=500)
        ...
        for name, tac in self.tactile_sensors.items():
            try:
                obs[name] = tac.async_read(timeout_ms=500)
            except (TimeoutError, RuntimeError):
                if tac.is_connected:
                    try:
                        obs[name] = tac.read_latest()
                    except: pass
    ...
    def get_observation_latest(self) -> dict:
        ...
        for name, cam in self.cameras.items():
            ...
            if cam.is_connected:
                try:
                    cam_data = cam.read_latest()
                except: pass
            
            if cam_data is None:
                # Fallback to zero-filled array of correct shape
                h, w = cam.config.height, cam.config.width
                cam_data = np.zeros((h, w, 3), dtype=np.uint8)
                logging.debug(f"Sensor '{name}' missing frame, providing zeros.")
            obs[name] = cam_data
        ...
        for name, tac in self.tactile_sensors.items():
            ...
            if tac.is_connected:
                try:
                    tac_data = tac.read_latest()
                except: pass
            
            if tac_data is None:
                shape = tac.config.expected_shape
                tac_data = np.zeros(shape, dtype=np.float32)
            obs[name] = tac_data
